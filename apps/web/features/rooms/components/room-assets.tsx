...
export function RoomAssets({ roomId }: { roomId: string }) {
  ...
  function openEdit(a: RoomAsset) { setEditing(a); setOpen(true); }

  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>Room Assets</CardTitle>
        <Button size="sm" onClick={openNew}><Plus className="h-4 w-4 mr-1" /> Add Asset</Button>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <EmptyState
            ...
            action={<Button size="sm" onClick={openNew}><Plus className="h-4 w-4 mr-1" /> Add Asset</Button>}
          />
        ) : (
          <div className="divide-y">
            {assets.map((a) => (
              <div key={a.id} className="flex items-center justify-between py-3">
                <div>
                  <div className="font-medium">{a.asset_name} <span className="text-muted-foreground">× {a.quantity}</span></div>
                ...
                <div className="flex gap-1">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => setToDelete(a)}><Trash2 className="h-4 w-4" /></Button>
                </div>
              </div>
            ))}
          </div>
        ...
        <DialogContent>
          ...
          <AssetForm
            ...
            onSubmit={async (v) => {
              ...
              else await create.mutateAsync({ ...v, room_id: roomId });
              setOpen(false);
            }}
          />
        </DialogContent>
...
function AssetForm({
  ...
  onSubmit: (v: {
    asset_name: string; quantity: number; condition: RoomAsset["condition"];
    status: RoomAsset["status"]; purchase_date?: string; notes?: string;
  }) => void | Promise<void>;
  onCancel: () => void;
  loading?: boolean;
}) {
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        onSubmit({ asset_name: assetName, quantity, condition, status, purchase_date: purchaseDate || undefined, notes: notes || undefined });
      }}
      className="space-y-4"
    >
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <Input value={assetName} onChange={(e) => setAssetName(e.target.value)} placeholder="Bed / Fan / Geyser" required />
        </div>
        <div className="space-y-1">
